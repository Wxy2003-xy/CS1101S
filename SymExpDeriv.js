function deriv_symbolic(exp, x) {
    ...
    : is_variable(exp)      // check for the variable in the exp
        ...
        : is_power(exp)
                ? make_product(make_product(make_power(base(exp)
                                                       , make_sub(power(exp), 1))
                                            , deriv_symbolic(base(exp), x))
                               , power(exp))
...
const exp5 = make_power(make_sum(2, "x"), 2);
// x^2 + x + 4
...
// x ^ 3 / 2 * x + 1
const exp6 = make_division(make_power("x", 3), make_sum(make_product(2, "x"), 1));

//display_list(deriv_symbolic(exp1, "x"));
//display_list(deriv_symbolic(exp2, "x"));
display_list(deriv_symbolic(exp6, "x"));
